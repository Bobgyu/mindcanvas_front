...
function Tree() {
  ...
  const analyzeDrawing = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/predict/Tree', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        // 분석 결과를 로컬 스토리지에 저장
        localStorage.setItem('treeAnalysisResult', JSON.stringify(result.detections))
        // 분석 페이지로 이동
